...
class ViewController: UIViewController,UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource {
    ...
    @IBOutlet weak var CViewBottom: NSLayoutConstraint!
    ...
    @IBAction func showBubbleMeasureScene(_ sender: Any) {
        if let bubbleLevelViewController = storyboard?.instantiateViewController(withIdentifier: "BubbleLevelViewController") as? BubbleLevelViewController {
            present(bubbleLevelViewController, animated: true, completion: nil)
        }
    }
    
    @IBAction func showARMeasurementScene(_ sender: Any) {
        if let rulerViewController = storyboard?.instantiateViewController(withIdentifier: "RulerViewController") as? RulerViewController {
            present(rulerViewController, animated: true, completion: nil)
        }
    }
    ...
    func textField(_ textField: UITextField, shouldChangeCharactersIn range: NSRange, replacementString string: String) -> Bool {
        if(textField.text!.count > 8) {return false}
        self.updateTriangleValues()

...
class LineNode: NSObject {
    ...
    var lineNodeColor: UIColor?
    
    convenience init(startPos: SCNVector3, sceneV: ARSCNView, cameraNode:SCNNode) {
        self.init(startPos: startPos, sceneV: sceneV, cameraNode: cameraNode, lineColor: .white)
    }
    
    init(startPos: SCNVector3, sceneV: ARSCNView, cameraNode:SCNNode, lineColor:UIColor) {
        sceneView = sceneV
        lineNodeColor = lineColor
        
        let dot = SCNSphere(radius:1)
        dot.firstMaterial?.diffuse.contents = UIColor.dot
    ...
    public func updatePosition(pos: SCNVector3, camera: ARCamera?) -> Float {
        ...
        if !length.isNaN {
            let lengthInFeet = length*Float.LengthUnit.Foot.rate.0
            text.string = Length.init().formattedStringForLine(feet: Double(lengthInFeet))
        }
        textNode.setPivot()
...
//        line.firstMaterial?.diffuse.contents = lineNodeColor
//        return SCNNode(geometry: line)
        
        return SCNNode().buildLineInTwoPointsWithRotation(from: nodeA.position, to: nodeB.position, radius: 1 / 1000.0, color: lineNodeColor!)

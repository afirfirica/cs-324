...
extension ARCamera.TrackingState {
    var presentationString: String {
        ...
        case .normal:
            return "Tracking as normal"
        case .limited(let reason):
            switch reason {
            case .excessiveMotion:
                return "Device moves too fast\nPlease move slowly"
            case .insufficientFeatures:
                return "Too few feature points\nPlease ensure that the camera is not blocked and ambient light is sufficient"
            case .initializing:
                return "AR is initializing\nMove the device left and right to get more feature points"
            }
        }
    }
}
...
extension ARError.Code {
    var presentationString: String {
        ...
        case .unsupportedConfiguration,.sensorUnavailable,.sensorFailed,.worldTrackingFailed:
            return "Unfortunately, your current device does not support it"
        case .cameraUnauthorized:
            return "Camera is failed to open"
        }
    }
}
